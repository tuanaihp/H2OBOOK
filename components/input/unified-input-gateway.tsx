...
import { AlertTriangle, CheckCircle2, FileArchive, FileImage, FileText, Globe2, Loader2, Play, RefreshCw, RotateCcw, Save, Upload, XCircle } from "lucide-react";
...
type SourceState = { kind: "file"; file: File } | { kind: "url"; url: string } | null;
...
export function UnifiedInputGateway({ initialBookId }: { initialBookId?: string }) {
  ...
  const ensureSession = async (overrideMode = mode, design = false) => {
    if (!source || !format) throw new Error("INPUT_SOURCE_REQUIRED");
  ...
  return <div className="unified-input-gateway">
    ...
    {!imageInspection && source && <div className="input-action-row"><button className="btn btn-primary" disabled={busy} onClick={() => void process()}>{busy ? <Loader2 className="spin"/> : <Play/>} Xử lý & tạo preview</button><button className="btn btn-secondary" disabled={busy} onClick={() => void cancel()}><XCircle/> Hủy</button>{session && ["failed","cancelled","recovery_required"].includes(session.status) && <button className="btn btn-secondary" onClick={() => void retry()}><RefreshCw/> Retry</button>}{session && <button className="btn btn-secondary" onClick={() => void recover()}><RotateCcw/> Recovery</button>}</div>}
