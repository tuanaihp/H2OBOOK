...
export function PublicHomeV3({ viewModel, hero }: { viewModel: PublicHomeViewModel; hero?: ReactNode }) {
  ...
  const sections: Record<PublicHomeSectionKey, ReactNode> = {
    ...
    "final-cta": <section className="h2o-public-final-cta" data-public-home-section="final-cta" key="final-cta"><div className="h2o-public-container"><div><ShieldCheck/><span>H2OBOOK LEARNING SYSTEM</span><h2>Bạn không cần học mọi thứ cùng lúc.<br/><em>Bạn cần một điểm bắt đầu đúng.</em></h2><p>Chọn lộ trình, khám phá nội dung phù hợp và để hệ thống theo dõi hành trình nghề nghiệp của bạn.</p><div><TrackedLink className="h2o-public-primary large" href={viewModel.config.conversion.primaryCtaHref} section="final-cta" action="primary">{viewModel.config.conversion.primaryCtaLabel} <ArrowRight/></TrackedLink><TrackedLink className="h2o-public-secondary large" href={viewModel.config.conversion.secondaryCtaHref} section="final-cta" action="secondary">{viewModel.config.conversion.secondaryCtaLabel}</TrackedLink></div></div><div className={styles.finalCore}><H2OBrainCore size="min(232px,24vw)" label={`${viewModel.config.socialProof.students}+ Learning Twin`}/></div></div></section>,
  };
