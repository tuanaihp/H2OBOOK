...
export function NeuralRouteTheme(){
  ...
  const showAmbient=surface==="public"||surface==="auth";
  useEffect(()=>{const root=document.documentElement;const reduced=window.matchMedia("(prefers-reduced-motion: reduce)").matches;const saveData=Boolean((navigator as Navigator&{connection?:{saveData?:boolean}}).connection?.saveData);const applicationSurface=surface!=="public"&&surface!=="auth";root.dataset.h2oNeural=enabled?"on":"off";root.dataset.h2oSurface=surface;root.dataset.h2oNeuralIntensity=preset.intensity;root.dataset.h2oNeuralMotion=!isGlobalNeuralMotionEnabled()||reduced||saveData||applicationSurface?"reduced":"full";root.style.setProperty("--h2o-neural-accent",preset.accent);root.style.setProperty("--h2o-neural-secondary",preset.secondary);return()=>{delete root.dataset.h2oSurface;delete root.dataset.h2oNeuralIntensity;root.style.removeProperty("--h2o-neural-accent");root.style.removeProperty("--h2o-neural-secondary");};},[enabled,preset,surface]);
  if(!enabled||!showAmbient||!preset.showAmbient)return null;return <NeuralAmbientLayer preset={preset}/>;
}
