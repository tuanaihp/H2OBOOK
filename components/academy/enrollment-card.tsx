  ...
  return <section id="academy-enrollment" className="h2o-enrollment-section">
    <div className="h2o-public-container h2o-enrollment-grid">
      ...
      <form className="h2o-enrollment-card" onSubmit={submit}>
        ...
        <div className="h2o-enrollment-fields"><label><span>Họ và tên</span><input required minLength={2} name="fullName" autoComplete="name" value={form.name} onChange={(event) => update("name", event.target.value)} placeholder="Nguyễn Minh Anh"/></label><label><span>Email</span><input required type="email" name="email" autoComplete="email" value={form.email} onChange={(event) => update("email", event.target.value)} placeholder="ban@example.com"/></label><label><span>Số điện thoại</span><input name="phone" type="tel" autoComplete="tel" value={form.phone} onChange={(event) => update("phone", event.target.value)} placeholder="09xx xxx xxx"/></label><label><span>Điều bạn muốn đạt được</span><input name="message" value={form.message} onChange={(event) => update("message", event.target.value)} placeholder="Mục tiêu học tập của bạn"/></label></div>
        <label className="h2o-enrollment-honeypot" aria-hidden="true">Website<input tabIndex={-1} name="website" autoComplete="off" value={form.website} onChange={(event) => update("website", event.target.value)}/></label>
        <label className="h2o-enrollment-consent"><input type="checkbox" name="consent" checked={form.consent} onChange={(event) => update("consent", event.target.checked)}/><span>Tôi đồng ý để H2OBOOK liên hệ và xử lý dữ liệu cho mục đích đăng ký học.</span></label>
