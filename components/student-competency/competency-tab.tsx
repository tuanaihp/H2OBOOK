...
interface CompetencySkillPoint { key: string; label: string; latestScore: number | null; trend30: number | null; trend60: number | null; trend90: number | null; evidenceCount: number; weakEvidenceCount: number }
...
export function CompetencyTab({ classId, roster, initialStudentId }: { classId: string; roster: { studentId: string; name: string }[]; initialStudentId?: string }) {
  ...
  const repeated = (profile ?? []).filter((skill) => skill.weakEvidenceCount >= 2).sort((a, b) => b.weakEvidenceCount - a.weakEvidenceCount);
  ...
  return <div className={styles.card}>
    ...
    <div className={styles.cardBody}>
      ...
      {!loading && profile && <>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}><div className={styles.listItem}><div><strong>Lỗi/kỹ năng yếu lặp lại</strong><small>{repeated.length ? repeated.slice(0, 4).map((skill) => `${skill.label} (${skill.weakEvidenceCount} lần)`).join(" · ") : "Chưa phát hiện lỗi lặp lại"}</small></div></div><div className={styles.listItem}><div><strong>Bài luyện tiếp theo</strong><small>{weaknesses.length ? `Ưu tiên luyện ${weaknesses.slice(0, 2).map((skill) => skill.label).join(" và ")}; nộp ảnh/video trước–sau để giảng viên đánh giá lại.` : "Duy trì bài luyện tổng hợp và tốc độ hoàn thiện."}</small></div></div></div>
