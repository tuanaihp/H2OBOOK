...
import { SKILL_CATALOG, type RubricCategory } from "@/lib/student-competency/types";
import styles from "./student-management-workspace.module.css";

type Category = RubricCategory;
...
export function SettingsTab() {
  ...
  const load = useCallback(async () => {
    ...
    const latest = next[0];
    const defaultTitle = category === "makeup_product" ? "Bộ tiêu chí ảnh sản phẩm Makeup chuyên nghiệp" : `Rubric ${category}`;
    const baseTitle = latest?.title.replace(/\s*·\s*phiên bản\s+\d+$/i, "") ?? defaultTitle;
    setTitle(`${baseTitle} · phiên bản ${next.length + 1}`);
    setQuickIssuesText((latest?.quickIssues ?? []).join("\n"));
    const defaultProductCriteria = MAKEUP_PRODUCT_IMAGE_RUBRIC.map((criterion) => ({
    ...
    setCriteria(latest?.criteria.map((criterion) => ({ title: criterion.title, description: criterion.description, maxScore: criterion.maxScore, required: criterion.required, skillKey: criterion.skillKey ?? null }))
      ?? (category === "makeup_product" ? defaultProductCriteria : []));
  }, [category]);
  ...
  const save = async () => {
    ...
    setMessage("Đã tạo phiên bản mới và đồng bộ sang không gian học viên. Buổi chưa tự chấm sẽ dùng rubric mới; dữ liệu buổi đã nộp vẫn giữ nguyên phiên bản cũ."); await load();
  };
  ...
  return <section className={styles.section}><div className={styles.sectionHead}><div><h2>Cài đặt tiêu chí</h2><p>Chính sách tốt nghiệp dùng chung và rubric được quản lý theo phiên bản để bảo toàn lịch sử điểm.</p></div><span className={styles.scoreTotal} data-valid={total === 100}>Tổng {total}/100</span></div>
    ...
    <div className={styles.sectionHead} style={{ marginTop: 18 }}><div><h2>Cấu hình rubric theo phiên bản</h2><p>Không sửa rubric cũ. Mỗi lần lưu tạo một phiên bản mới và tự đồng bộ sang các buổi học viên chưa tự chấm.</p></div></div>
    <article className={styles.card}><div className={styles.cardBody}><div className={styles.settingsToolbar}><label>Nhóm rubric<select value={category} onChange={(event) => setCategory(event.target.value as Category)}><option value="training">Training</option><option value="makeup">Makeup</option><option value="hair">Hair</option><option value="makeup_product">Sản phẩm ảnh Makeup</option></select></label><label>Tên phiên bản<input value={title} onChange={(event) => setTitle(event.target.value)} /></label><span>{rubrics.length} phiên bản hiện có</span></div>
