...
export function TrainingGradingTab({ classId, organizationId, roster, initialStudentId }: { classId: string; organizationId?: string; roster: { studentId: string; name: string }[]; initialStudentId?: string }) {
  return <GradingForm classId={classId} organizationId={organizationId} roster={roster} initialStudentId={initialStudentId} category="training" sessionTypeFilter={["training_makeup_hair", "training_hair"]}
