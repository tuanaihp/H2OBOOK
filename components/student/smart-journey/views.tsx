...
export function JourneyMapView({ model, onMissionClick }: { model: SmartJourneyModel; onMissionClick: (id: string) => void }) {
  return <div>
...
export function JourneyRoadmapView({ model, onMissionClick }: { model: SmartJourneyModel; onMissionClick: (id: string) => void }) {
  ...
  return <div>
    {groups.filter((g) => g.rows.length).map((group) => <div key={group.label} style={{ marginBottom: 18 }}>
      ...
      {group.rows.map(({ outcome, mission }) => <button key={mission.id} className="h2o-sr-tlrow" disabled={mission.state === "locked"} onClick={() => onMissionClick(mission.id)} style={mission.state === "locked" ? { opacity: 0.6, cursor: "default" } : undefined}>
...
export function MissionControlListView({ model, onMissionClick, onFilterChanged }: { model: SmartJourneyModel; onMissionClick: (id: string) => void; onFilterChanged: () => void }) {
  ...
  return <div>
    ...
    {mode === "queue"
      ? <div className="h2o-sr-panel">{queue.length === 0
          ...
          : queue.map(({ mission }, i) => <button key={mission.id} className="h2o-sj-queuerow" onClick={() => onMissionClick(mission.id)}>
                ...
                <table className="h2o-sr-table">
                  ...
                  <tbody>
                    {rows.map(({ mission }) => <tr key={mission.id}>
                      ...
                      <td><button disabled={mission.state === "locked"} onClick={() => onMissionClick(mission.id)} className="h2o-sr-btn" style={{ fontSize: 11, padding: "5px 9px" }}>{actionLabel(mission)}</button></td>
                    </tr>)}
                  </tbody>
                </table>
              </div>
