...
export function StudentShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="h2o-student-shell">
    <aside className="h2o-student-sidebar">
      ...
      <div className="h2o-student-profile-mini"><div>{activeStudent?.name?.split(" ").slice(-1)[0]?.slice(0,1) ?? "H"}</div><span><small>Học viên</small><strong>{activeStudent?.name ?? "H2O Student"}</strong><em>{activeStudent?.progress ?? 68}% hành trình</em></span></div>
