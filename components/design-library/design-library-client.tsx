...
import { Award, BadgePercent, BookOpenCheck, Facebook, Filter, LayoutTemplate, Search, Sparkles, UserRound, WandSparkles } from "lucide-react";
    ...
    <div className={styles.library}>
      <section className={styles.hero}>
        ...
        <div className={styles.heroCopy}>
          ...
          <h1>Thư viện thiết kế dành riêng cho nghề Makeup</h1>
          <p>Tạo cover Fanpage, profile cá nhân, thiệp mời học viên, bằng tốt nghiệp và chương trình khuyến mãi — đồng bộ Brand Kit và mở trực tiếp trong Editor.</p>
          <div className={styles.heroActions}><button onClick={() => setCategory("fanpage-cover")}><Sparkles size={17}/>Khám phá mẫu nổi bật</button><Link href="/brand-kit"><LayoutTemplate size={17}/>Cập nhật Brand Kit</Link></div>
        </div>
        <div className={styles.heroStats}>
          ...
          <article><strong>Bulk</strong><span>Tạo bằng/thiệp hàng loạt</span></article>
        </div>
      </section>
      ...
      <section className={styles.templateGrid}>{filtered.map((template) => <article className={styles.templateCard} key={template.id}>
        ...
        <div className={styles.cardBody}>
          ...
          <div className={styles.cardFooter}><div><strong>{template.trendScore ?? 0}</strong><span>Trend score</span></div><div><strong>{template.supportedFormats.length}</strong><span>Kích thước</span></div>{template.bulkCapable && <div><strong>CSV</strong><span>Hàng loạt</span></div>}<button onClick={() => setSelected(template)}>Dùng mẫu</button></div>
        </div>
      </article>)}</section>

      {selected && <DesignConfigurator template={selected} brands={store.brands} activeBrandId={store.activeBrandId} onClose={() => setSelected(null)} onCreate={createDesign}/>} 
    </div>
  </AppShell>;
}
