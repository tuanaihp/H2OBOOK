...
export default function StudentDashboard() {
  ...
  return <div className={styles.surface}>
    <section className={styles.hero}>
      <span className={styles.eyebrow}>{l("HÔM NAY · KHÔNG GIAN HỌC TẬP", "TODAY · LEARNING WORKSPACE")}</span>
      <h1>{l("Chào", "Hello")}, {name} 👋</h1>
      <p>{next ? l("Bước quan trọng nhất lúc này: ", "Your most important next step: ") + next.title : l("Theo dõi tiến độ, mở lịch học và tiếp tục hành trình của bạn.", "Track your progress, open your schedule and continue your journey.")}</p>
    ...
    <div className={styles.columns}>
      ...
      <section className={styles.panel}><h2>{l("Tiếp tục học", "Continue learning")}</h2><p>{data?.nextCourse?.title ?? l("Chọn khóa học được cấp quyền để bắt đầu hoặc tiếp tục học.", "Choose an available course to start or continue learning.")}</p><div className={styles.actions}><Link className={styles.primary} href={courseHref}><BookOpen size={16}/>{l("Mở khóa học", "Open courses")}</Link></div><hr/><div className={styles.assistHeader}><Sparkles size={18}/><div><h2>{l("Hỗ trợ theo ngữ cảnh", "Contextual help")}</h2><p>{l("Hỏi về bài đang học, xem nguồn và tạo câu hỏi ôn tập. Khi AI tắt, gợi ý cục bộ vẫn hoạt động.", "Ask about your lesson, view sources and create review prompts. Local guidance remains available when AI is off.")}</p></div></div><div className={styles.actions}><Link href="/student/mentor">{l("Hỏi trợ lý học tập", "Ask learning assistant")}</Link><Link href="/student/library">{l("Tìm tài liệu tham khảo", "Find reference material")}</Link></div></section>
    </div>
    <section className={styles.panel}><header><div><h2>{l("Kỹ năng cần củng cố", "Skills to strengthen")}</h2><p>{l("Mức thành thạo chỉ thay đổi khi có bài học hoặc bằng chứng thực hành được ghi nhận.", "Mastery changes only when learning or practice evidence is recorded.")}</p></div><Link href="/student/roadmap">{l("Xem lộ trình", "View roadmap")}</Link></header><div className={styles.modules}>{data?.skillMastery.length ? data.skillMastery.map(skill => <article className={styles.module} key={skill.key}><Target size={20}/><h3>{skill.label}</h3><b>{skill.masteryPercent}%</b><progress className={styles.progress} max={100} value={Math.max(0, Math.min(100, skill.masteryPercent))} aria-label={skill.label}/>{skill.nextAction && <p>{skill.nextAction}</p>}<small><CheckCircle2 size={13}/>{l("Bước tiếp theo được đề xuất", "Suggested next step")}</small></article>) : <p className={styles.empty}>{l("Kỹ năng được cập nhật từ bài học và kết quả thực hành của bạn.", "Skills are updated from your lessons and practice results.")}</p>}</div></section>
