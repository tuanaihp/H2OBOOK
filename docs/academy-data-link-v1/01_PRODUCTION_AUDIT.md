...
**Fix:** `findMissionStage()` trả thêm `position`; Mission Workspace badge đổi sang `Giai đoạn {String(stage.position + 1).padStart(2,"0")}` (cùng công thức `.position`-based với Roadmap, không dùng `index_label` nữa). `/student/library` cũng đổi nhãn section sang cùng công thức cho nhất quán. `index_label` vẫn giữ trong schema (admin có thể dùng làm ghi chú nội bộ) nhưng không còn là nguồn cho bất kỳ badge nào hiện ra với học viên.
